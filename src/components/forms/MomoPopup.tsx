'use client';

import { motion, AnimatePresence } from 'framer-motion';

interface MomoInfo {
  momoNumero: string;
  momoNom: string;
  momoWhatsapp: string;
  montant?: number;
  prenoms?: string;
  email?: string;
}

interface Props {
  open: boolean;
  onClose: () => void;
  info: MomoInfo;
}

export default function MomoPopup({ open, onClose, info }: Props) {
  const whatsappText = encodeURIComponent(
    `Bonjour, je viens d'effectuer un paiement de ${info.montant?.toLocaleString('fr-FR') ?? ''} FCFA pour la formation Premier Pas Vers Le Jeu.\nNom : ${info.prenoms ?? ''}\nEmail : ${info.email ?? ''}`
  );
  const whatsappUrl = `https://wa.me/${info.momoWhatsapp?.replace(/\D/g, '')}?text=${whatsappText}`;

  return (
    <AnimatePresence>
      {open && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          onClick={(e) => e.target === e.currentTarget && onClose()}
        >
          <motion.div
            initial={{ scale: 0.9, opacity: 0, y: 20 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            exit={{ scale: 0.9, opacity: 0, y: 20 }}
            transition={{ type: 'spring', damping: 20 }}
            className="bg-[#111] border border-orange-500/20 rounded-2xl p-6 w-full max-w-md relative"
          >
            {/* Close button */}
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-gray-500 hover:text-white transition-colors w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/10"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>

            <div className="text-center mb-6">
              <div className="w-14 h-14 bg-orange-500/20 rounded-full flex items-center justify-center text-2xl mx-auto mb-3">
                📱
              </div>
              <h3 className="text-white font-black text-xl">Paiement par MoMo</h3>
              <p className="text-gray-400 text-sm mt-1">Le paiement en ligne n'a pas pu être traité. Effectuez un virement direct.</p>
            </div>

            {/* MoMo info */}
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3 mb-5">
              <div className="flex items-center justify-between">
                <span className="text-gray-500 text-sm">Numéro MoMo</span>
                <span className="text-white font-bold text-lg tracking-wide">{info.momoNumero}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-500 text-sm">Nom du compte</span>
                <span className="text-white font-semibold">{info.momoNom}</span>
              </div>
              {info.montant && (
                <div className="flex items-center justify-between border-t border-white/10 pt-3">
                  <span className="text-gray-500 text-sm">Montant à envoyer</span>
                  <span className="text-orange-400 font-black text-xl">{info.montant.toLocaleString('fr-FR')} FCFA</span>
                </div>
              )}
            </div>

            {/* Steps */}
            <div className="space-y-2 mb-5">
              <p className="text-gray-500 text-xs uppercase tracking-widest font-semibold">Étapes</p>
              {[
                '1. Effectuez le virement MoMo au numéro ci-dessus',
                '2. Prenez une capture d\'écran de la confirmation',
                '3. Envoyez-la par WhatsApp avec votre nom',
              ].map((step) => (
                <div key={step} className="flex items-start gap-2 text-sm text-gray-400">
                  <span className="text-orange-500 mt-0.5">›</span>
                  <span>{step}</span>
                </div>
              ))}
            </div>

            {/* WhatsApp button */}
            {info.momoWhatsapp && (
              <a
                href={whatsappUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-xl text-sm transition-colors mb-3"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
                Envoyer la capture sur WhatsApp
              </a>
            )}

            <button
              onClick={onClose}
              className="w-full border border-white/10 text-gray-400 hover:text-white py-2.5 rounded-xl text-sm transition-colors"
            >
              Fermer
            </button>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
  );
}
